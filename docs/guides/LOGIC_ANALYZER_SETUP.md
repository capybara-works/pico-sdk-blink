...
実機では GND + D2→GP0 + D0→GP5 + D1→GP4 を接続し、`PICO_LOGIC_ANALYZER=1` で
...
- **生トグルで各チャネルの活性を見る**(デコード前の一次切り分け):
  ```bash
  sigrok-cli --driver fx2lafw --config samplerate=1000000 --time 6000 \
    --channels D0,D1,D2,D3 -O bits
  ```
  各チャネルのトグル数を数える。**SDA(D1)は活発なのにSCL(D0)がidle-LOWでトグル0**なら、
  I2CはSCL/SDAが必ず一緒に動くので、SCLタップ(D0)が届いていない物理的証拠 → 挿し直す。
