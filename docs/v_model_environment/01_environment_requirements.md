...
### 2.2 目標
*   **再現性**: DevContainerとCIで主要ツールを揃え、同じ入口スクリプトでビルド・検証できること。手動ローカル環境の差分はログで確認できること。
...
*   **REQ-E-006**: UARTログ出力をテストスクリプトから観測・検証できること。GPIO状態は補助ツールまたは将来のHIL拡張で扱う。

### 3.4 CI/CDパイプライン機能
*   **REQ-E-007**: GitHub へのプッシュをトリガーとして、自動的にビルドとテストが実行されること。
*   **REQ-E-008**: ローカル環境とCI環境で、共通のビルド・テスト入口スクリプトが使用できること。
