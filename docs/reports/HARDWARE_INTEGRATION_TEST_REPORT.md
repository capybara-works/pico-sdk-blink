...
> [!NOTE]
> 現行状態注記 (2026-06-13): この文書は2025-12時点の実機検証記録です。
> 現在は `scripts/verify_all.sh` を標準入口とし、`scripts/run_hil.sh` が
> `tools/hil/hil_runner.py` を呼び出します。本文の「今後の展開」にある
> HIL Runner作成は一部完了済みですが、`expect-pin` のシナリオ統合、
> 複数サイクル・周期検証、step単位の構造化evidenceは引き続き改善余地です。
